...
window.onload = function init() {
	...
	canvas.addEventListener("contextmenu", function (event) {
		...
		if(rightClick && index >=0){
		    ...
		    index = findPoint(t);
			dotArr.splice(index,numOfTris);// cut off all points of triangle fan
			dotCols.splice(index,numOfTris);
			index=-1;	
		}
...
function render() {
    ...
    for(var i = 0; i <= dotArr.length; i+=numOfTris){
